...
namespace MicroserviceExplorer
{
	public class NewMicroserviceCreator
	{
		...
		public static async Task<bool> CreateMicroServiceAsync(string SolutionPath, string ProjectName)
		{
			ProcessStartInfo processStartInfo = new ProcessStartInfo()
			{
				WorkingDirectory = SolutionPath,
				FileName = "msharp-build",
				Arguments = string.Format("/new-ms /n:\"{0}\"", ProjectName),
				CreateNoWindow = true,
			...
			try
			{
				var MicroCreateProc = Process.Start(processStartInfo);
				MicroCreateProc.WaitForExit();
			}
			catch
			{
