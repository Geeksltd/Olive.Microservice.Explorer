...
namespace MicroserviceExplorer
{
	...
	public sealed class MicroserviceItem : INotifyPropertyChanged
	{
		...
		public List<NugetReference> OldReferences => References.Except(x => x != null && x.IsUpToDate).ToList();
		...
		public void RefreshPackages()
		{
			...
			Parallel.ForEach(StandardProjects, project =>
			{
				var Pkgs = project.GetNugetPackages(SolutionFolder);
				IEnumerable<(string name, string ver)> packages = new List<(string name, string ver)>();
				if (Pkgs != null)
				{
					packages = Pkgs.Except(x => x.name.StartsWith("Microsoft.AspNetCore."));
				}
				Parallel.ForEach(packages, Package =>
				{
					try
					{
						if (!References.Any(x => x != null && x.Version == Package.ver && x.Name == Package.name && x.Project == project && x.Service == this))
							References.Add(new NugetReference(Package.name, Package.ver, this, project));
					}
					catch
					{
